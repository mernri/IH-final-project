...
// POST Route - JOIN a TRIBE
authRoutes.post("/jointribe", (req, res, next) => {
  ...
  User.findOneAndUpdate({ _id: userId }, { tribe: tribeId })
    .then(theUser => {
      console.log(theUser);
      res.json(theUser);
    })
...
authRoutes.post("/leavetribe", (req, res, next) => {
  ...
  User.findOneAndUpdate({ _id: userId }, { $unset: { tribe: "" } })
    .then(theUser => {
      console.log(theUser);
      res.json(theUser);
    })
